...
inline constexpr size_t kMaxFragment =
    kMaxFramePayload - kFrameHeaderLen; // 225
...
enum class OtaFrameType : uint8_t {
  Start =
      1, // seeder->target: session geometry (a hint; manifest is authoritative)
  ...
  Request =
      5,     // target->seeder: please send <index> (kIndexManifest or a block)
  Ack = 6,   // target->seeder: <index> verified
  Done = 7,  // target->seeder: all blocks verified
  Abort = 8, // either side: session aborted
  Load =
      9, // client->relay: a chunk of the package to seed (OtaLoadInfo prefix)
...
// 8-byte little-endian frame header. Field meaning is per-type; see the
// senders.
...
// when the destination is the inactive slot and the active slot is untouched.
// The delta path uses a buffering store + the detools decoder instead; this
// bridge is not used there.
//
...
// path: the received payload is a detools delta (tens of KB), assembled here
// and then decoded against the base into the flash target. Blocks may arrive in
// any order (random-access into the buffer); resume is RAM-only (restart on
// reboot).
...
// Receiver-side FSM. Drives manifest receipt, per-block verification against
// the merkle root, journaling, and the pull/ACK requests back to the seeder.
class OtaReceiver {
public:
...
private:
  ...
  std::map<uint16_t, std::vector<uint8_t>>
      ready_blocks_; // complete, awaiting proof
...
class OtaSeeder {
public:
  OtaSeeder(const uint8_t *package, size_t package_len, IFrameOut &out,
            uint8_t session, size_t fragment_size = 200);
