...
struct FakeVerifier : ISignatureVerifier {
  ...
  bool verify(const uint8_t *, size_t, const uint8_t *sig,
    ...
    return sig_len == good.size() &&
           std::memcmp(sig, good.data(), sig_len) == 0;
  }
};
...
static Manifest make_manifest(const std::vector<uint8_t> &payload,
                              uint32_t block_size, const char *env,
                              const char *ver, const char *commit) {
  Manifest m;
...
static BuiltPackage build_package(const Manifest &m,
...
static void test_parse_ok() {
  ...
  Manifest m =
      make_manifest(payload, 1024, "heltec-v3", "2.8.0.54e0d8d", "54e0d8d");
  auto pkg = build_package(m, payload, sig);

  ParsedPackage parsed;
  TEST_ASSERT_EQUAL_INT(static_cast<int>(PackageError::Ok),
                        static_cast<int>(package_parse(
                            pkg.bytes.data(), pkg.bytes.size(), parsed)));
...
static void test_parse_too_small() {
  ...
  TEST_ASSERT_EQUAL_INT(
      static_cast<int>(PackageError::TooSmall),
...
static void test_parse_bad_magic() {
  ...
  auto pkg =
      build_package(make_manifest(payload, 64, "e", "v", "c"), payload, sig);
...
static void test_parse_payload_length_mismatch() {
  ...
  auto sig = std::vector<uint8_t>(kEd25519SigLen, 0);
  auto pkg =
      build_package(make_manifest(payload, 256, "e", "v", "c"), payload, sig);
...
static void test_signature_verify() {
  ...
  auto pkg =
      build_package(make_manifest(payload, 512, "e", "v", "c"), payload, good);
  ParsedPackage parsed;
...
static void test_identity_gate() {
  ...
  auto sig = std::vector<uint8_t>(kEd25519SigLen, 0);
  Manifest m =
      make_manifest(payload, 64, "heltec-v3", "2.8.0.54e0d8d", "54e0d8d");
...
static void test_end_to_end_block_verify() {
  ...
  for (uint32_t off = 0; off < parsed.payload_len; off += bs) {
    uint32_t n =
        (off + bs <= parsed.payload_len) ? bs : parsed.payload_len - off;
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    TEST_ASSERT_TRUE(
        merkle_verify_block(parsed.payload + off, n, i, count, proof, root));
  }
}
