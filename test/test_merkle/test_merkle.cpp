...
static std::vector<std::vector<uint8_t>> make_blocks(size_t count, size_t block_size) {
    std::vector<std::vector<uint8_t>> blocks;
    for (size_t i = 0; i < count; ++i) {
        blocks.push_back(ota_test::prng_bytes(block_size, 0x1000 + static_cast<uint32_t>(i)));
    }
    return blocks;
}

static std::vector<Hash256> leaves_of(const std::vector<std::vector<uint8_t>>& blocks) {
    std::vector<Hash256> leaves;
    for (const auto& b : blocks) leaves.push_back(hash_leaf(b.data(), b.size()));
    return leaves;
}
    ...
    for (size_t count = 1; count <= 33; ++count) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            TEST_ASSERT_TRUE_MESSAGE(
                merkle_verify_block(blocks[i].data(), blocks[i].size(), i, count, proof, root),
...
static void test_wrong_block_fails() {
    ...
    corrupt[0] ^= 0x01;
    TEST_ASSERT_FALSE(merkle_verify_block(corrupt.data(), corrupt.size(), 3, 7, proof, root));
}
...
static void test_index_out_of_range() {
    ...
    Hash256 root = merkle_root(leaves);
    auto proof = merkle_proof(leaves, 0);
    TEST_ASSERT_FALSE(merkle_verify(leaves[0], 4, 4, proof, root)); // index == count
...
static void test_short_final_block() {
    ...
    Hash256 root = merkle_root(leaves);
    auto proof = merkle_proof(leaves, 4);
    TEST_ASSERT_TRUE(merkle_verify_block(blocks[4].data(), blocks[4].size(), 4, 5, proof, root));
}
