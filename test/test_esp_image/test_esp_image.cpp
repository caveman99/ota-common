...
static std::vector<uint8_t> make_esp_image(const std::vector<uint32_t>& seg_lens,
                                           bool hash_appended) {
    ...
    if (hash_appended) img.insert(img.end(), 32, 0xAB);
    return img;
}
...
static void test_locate_trailer_after_image() {
    ...
    OtaTrailer got;
    TEST_ASSERT_TRUE(r.read(parsed_len, reinterpret_cast<uint8_t*>(&got), sizeof(got)));
