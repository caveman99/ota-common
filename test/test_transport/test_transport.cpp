...
struct FakeVerifier : ISignatureVerifier {
  ...
  bool ok = true;
  bool verify(const uint8_t *, size_t, const uint8_t *sig,
...
static std::vector<uint8_t> build_package(const std::vector<uint8_t> &payload,
                                          uint16_t block_size,
  ...
  m.block_count =
      static_cast<uint32_t>((payload.size() + block_size - 1) / block_size);

  std::vector<Hash256> leaves;
  for (size_t off = 0; off < payload.size(); off += block_size) {
    size_t n = (off + block_size <= payload.size()) ? block_size
                                                    : payload.size() - off;
...
struct MemStore : IBlockStore {
  ...
  bool has_block(uint32_t i) const override {
    return blocks.find(i) != blocks.end();
  }
  bool put_block(uint32_t i, const uint8_t *d, uint32_t n) override {
    blocks[i].assign(d, d + n);
    return true;
  }
  uint32_t stored_count() const override {
...
struct Endpoint : IFrameOut {
  ...
  void send_frame(const uint8_t *d, size_t n) override {
    outbox.emplace_back(d, d + n);
  }
};
...
// mutate(frame) is applied to each seeder->receiver frame (for fault
// injection).
static int pump(OtaSeeder &seeder, Endpoint &seederEp, OtaReceiver &receiver,
                Endpoint &recvEp,
                std::function<void(std::vector<uint8_t> &)> mutate = nullptr,
                int cap = 200000) {
  int delivered = 0;
...
static void run_transfer(const std::vector<uint8_t> &payload,
                         uint16_t block_size, size_t frag, MemStore &store) {
  ...
  std::map<uint16_t, int> blockSends;
  {
    ...
    while (!sEp.outbox.empty() || !rEp.outbox.empty()) {
      ...
      if (!sEp.outbox.empty()) {
        ...
        OtaFrameHeader h;
        if (frame_header_unpack(f.data(), f.size(), h) &&
            h.type == OtaFrameType::Block)
          blockSends[h.index]++;
        receiver.on_frame(f.data(), f.size());
      }
      if (!rEp.outbox.empty()) {
...
static void test_corruption_recovery() {
  ...
  auto mutate = [&](std::vector<uint8_t> &f) {
    ...
    if (!frame_header_unpack(f.data(), f.size(), h))
      return;
    if (!corrupted && h.type == OtaFrameType::Block && h.index == 2 &&
        h.off == 0 && f.size() > kFrameHeaderLen) {
...
static void test_geometry_mismatch_aborts() {
  ...
  OtaStartInfo bad{1024, 99 /*wrong*/, static_cast<uint32_t>(payload.size()),
                   192, 64};
  ...
  // Now feed the real manifest (single fragment, it fits):
  // header(8)+manifest+sig.
  const size_t total = sizeof(Manifest) + kSig.size();
  std::vector<uint8_t> mf(kFrameHeaderLen + total);
  OtaFrameHeader mh{OtaFrameType::Manifest, 5, kIndexManifest, 0,
                    static_cast<uint16_t>(total)};
  frame_header_pack(mh, mf.data());
  std::memcpy(mf.data() + kFrameHeaderLen, pkg.data() + sizeof(PackageHeader),
              total);
