...
static Hash256 root_over_payload(const ParsedPackage &pkg) {
  ...
  for (uint32_t off = 0; off < pkg.payload_len; off += bs) {
    uint32_t n = (off + bs <= pkg.payload_len)
                     ? bs
                     : pkg.payload_len - static_cast<uint32_t>(off);
...
static void test_full_package() {
  ...
  TEST_ASSERT_EQUAL_INT(
      static_cast<int>(PackageError::Ok),
  ...
  TEST_ASSERT_EQUAL_MEMORY(pkg.manifest->payload_merkle_root, root.bytes,
                           kSha256Len);
...
static void test_delta_package() {
  ...
  TEST_ASSERT_TRUE(pkg.payload_len <
                   pkg.manifest->output_length); // delta < image
...
static void test_identity_gate_against_base_trailer() {
  ...
  TEST_ASSERT_TRUE(
      trailer_from_image_tail(target.data(), target.size(), target_tr));
