...
static ParsedPackage parse(const std::vector<uint8_t>& pkg) {
    ParsedPackage p;
    TEST_ASSERT_EQUAL_INT((int)PackageError::Ok, (int)package_parse(pkg.data(), pkg.size(), p));
    return p;
}
...
// the decoder errors, or the output fails the hard hash gate. This is what makes
// a missing base-flash hash safe (the catch is at the output).
static void test_wrong_base_fails_output_gate() {
    ...
    bool matches_target = out_size > 0 && sink.out.size() == target.size() &&
    ...
    if (out_size > 0) {
        uint8_t h[kSha256Len];
        Sha256::hash(sink.out.data(), sink.out.size(), h);
        TEST_ASSERT_NOT_EQUAL(0, std::memcmp(pkg.manifest->output_sha256, h, kSha256Len));
    }
}
...
static void test_delta_into_flash_target() {
    ...
    TEST_ASSERT_EQUAL_INT((int)FlashStatus::Ok,
...
struct RamFlash : IFlashMem {
    ...
    int read(uintptr_t src, void* dst, size_t len) override {
        if (src + len > mem.size()) return -1;
        std::memcpy(dst, mem.data() + src, len);
        return 0;
    }
    int write(uintptr_t dst, const void* src, size_t len) override {
        if (dst + len > mem.size()) return -1;
        if (write_budget == 0) return -1; // simulated power loss
...
static RamFlash load_region(const std::vector<uint8_t>& base) {
    ...
    std::memcpy(f.mem.data(), base.data(), base.size()); // from-image at the start
    return f;
}
...
// Power loss mid-apply, then resume: the journal persists the step, and a second
// apply over the same (partially patched) region completes to the target.
