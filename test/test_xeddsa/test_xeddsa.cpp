// Cross-language XEdDSA: verify a signature the Python `xeddsa` package produced
// (an independent Signal-XEdDSA implementation) with the vendored ota-common
// verifier. This proves the ota-common verify matches the scheme the admin
// device / firmware use, and closes the loop with tools/gen_xeddsa_fixtures.py.
...
static void test_xeddsa_interop_ok() {
    ...
    TEST_ASSERT_TRUE(xeddsa_verify(pub.data(), 0, 0, kOtaPortnum, payload.data(), payload.size(),
                                   sig.data()));
}
...
static void test_tampered_payload_fails() {
    ...
    auto sig = read_fixture("xeddsa_sig.bin");
    payload[10] ^= 0x01;
    TEST_ASSERT_FALSE(xeddsa_verify(pub.data(), 0, 0, kOtaPortnum, payload.data(), payload.size(),
                                    sig.data()));
}
...
static void test_wrong_portnum_fails() {
    ...
    auto sig = read_fixture("xeddsa_sig.bin");
    TEST_ASSERT_FALSE(
        xeddsa_verify(pub.data(), 0, 0, 6 /*ADMIN_APP*/, payload.data(), payload.size(), sig.data()));
}
...
static void test_wrong_key_fails() {
    ...
    auto payload = read_fixture("xeddsa_payload.bin");
    auto sig = read_fixture("xeddsa_sig.bin");
    TEST_ASSERT_FALSE(xeddsa_verify(other.data(), 0, 0, kOtaPortnum, payload.data(), payload.size(),
                                    sig.data()));
}
...
static void test_admin_key_verifier() {
    ...
    TEST_ASSERT_FALSE(empty.verify(payload.data(), payload.size(), sig.data(), sig.size()));

    AdminKeyVerifier one;
    one.add_key(pub.data());
    TEST_ASSERT_TRUE(one.verify(payload.data(), payload.size(), sig.data(), sig.size()));
    ...
    TEST_ASSERT_TRUE(multi.verify(payload.data(), payload.size(), sig.data(), sig.size()));

    // Only the wrong key -> reject.
    AdminKeyVerifier wrong;
    wrong.add_key(other.data());
    TEST_ASSERT_FALSE(wrong.verify(payload.data(), payload.size(), sig.data(), sig.size()));
