...
bool trailer_is_valid(const OtaTrailer& t) {
    if (t.magic != kTrailerMagic) return false;
    if (t.format != kTrailerFormat) return false;
    const auto* bytes = reinterpret_cast<const uint8_t*>(&t);
    const uint32_t expect = crc32_ieee(bytes, sizeof(OtaTrailer) - sizeof(uint32_t));
    return expect == t.crc32;
}

bool trailer_from_image_tail(const uint8_t* image, size_t image_len, OtaTrailer& out) {
    if (image == nullptr || image_len < sizeof(OtaTrailer)) return false;
