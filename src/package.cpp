...
PackageError package_parse(const uint8_t* buf, size_t buf_len, ParsedPackage& out) {
    out = ParsedPackage{};
    if (buf == nullptr || buf_len < sizeof(PackageHeader)) return PackageError::TooSmall;

    const auto* hdr = reinterpret_cast<const PackageHeader*>(buf);
    if (hdr->magic != kPackageMagic) return PackageError::BadMagic;
    if (hdr->format != kPackageFormat) return PackageError::BadFormat;
    if (hdr->manifest_length != sizeof(Manifest)) return PackageError::BadManifest;
    if (hdr->signature_length != kEd25519SigLen) return PackageError::BadManifest;

    // Compute section offsets with overflow-safe accumulation.
    size_t off = sizeof(PackageHeader);
    if (hdr->manifest_length > buf_len - off) return PackageError::LengthMismatch;
    const auto* manifest = reinterpret_cast<const Manifest*>(buf + off);
    off += hdr->manifest_length;

    if (hdr->signature_length > buf_len - off) return PackageError::LengthMismatch;
    ...
    if (manifest->magic != kManifestMagic) return PackageError::BadManifest;
    if (manifest->format != kManifestFormat) return PackageError::BadManifest;
    if (manifest->payload_length != payload_len) return PackageError::LengthMismatch;
...
bool package_verify_signature(const ParsedPackage& pkg, const ISignatureVerifier& verifier) {
    if (pkg.manifest == nullptr || pkg.signature == nullptr) return false;
