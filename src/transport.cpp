...
bool frame_header_unpack(const uint8_t* in, size_t len, OtaFrameHeader& out) {
    if (in == nullptr || len < kFrameHeaderLen) return false;
...
bool start_info_unpack(const uint8_t* in, size_t len, OtaStartInfo& out) {
    if (in == nullptr || len < 12) return false;
...
bool OtaReceiver::Reassembler::feed(uint16_t off, const uint8_t* data, size_t len) {
    if (!active) return false;
    if (off != filled) return false;          // require in-order fragments
    if (off + len > total) return false;       // overrun guard
    if (len) std::memcpy(buf.data() + off, data, len);
...
uint16_t OtaReceiver::next_needed_block() const {
    for (uint16_t i = 0; i < block_count_; ++i) {
        if (!store_.has_block(i)) return i;
    }
...
void OtaReceiver::on_frame(const uint8_t* data, size_t len) {
    OtaFrameHeader h;
    if (!frame_header_unpack(data, len, h)) return;
    if (h.session != session_) return;
...
void OtaReceiver::poll() {
    if (!aborted_ && !complete_) request_next();
}

void OtaReceiver::handle_start(const OtaFrameHeader&, const uint8_t* p, size_t n) {
    OtaStartInfo s;
    if (!start_info_unpack(p, n, s)) return;
...
void OtaReceiver::handle_manifest(const OtaFrameHeader& h, const uint8_t* p, size_t n) {
    if (manifest_ready_) return;
    if (h.off == 0) manifest_re_.reset(h.total);
    if (!manifest_re_.feed(h.off, p, n)) return;
...
void OtaReceiver::handle_block(const OtaFrameHeader& h, const uint8_t* p, size_t n) {
    if (!manifest_ready_) return;            // need the root before trusting blocks
    if (store_.has_block(h.index)) return;   // already journaled

    if (h.off == 0 || block_in_flight_ != h.index) {
        block_re_.reset(h.total);
        block_in_flight_ = h.index;
    }
    if (!block_re_.feed(h.off, p, n)) return;
...
void OtaReceiver::handle_proof(const OtaFrameHeader& h, const uint8_t* p, size_t n) {
    if (!manifest_ready_) return;
    if (store_.has_block(h.index)) return;
    ...
    if (!proof_re_.feed(h.off, p, n)) return;
...
void OtaReceiver::try_finish_block(uint16_t index) {
    ...
    if (bit == ready_blocks_.end() || pit == proofs_.end()) return;

    const std::vector<uint8_t>& block = bit->second;
    const bool ok = merkle_verify_block(block.data(), block.size(), index, block_count_, pit->second,
                                        merkle_root_);
    if (ok) {
...
bool FlashTargetBlockStore::put_block(uint32_t index, const uint8_t* data, uint32_t len) {
    if (manifest_ == nullptr) return false;

    if (!began_) {
        if (target_.begin(manifest_->output_length, *manifest_) != FlashStatus::Ok) return false;
        began_ = true;
    ...
    if (index < have_.size() && have_[index]) return true; // idempotent
    if (index != next_) return false;                       // require sequential

    if (target_.write(index * block_size_, data, len) != FlashStatus::Ok) return false;
    have_[index] = true;
    ++next_;
    ++count_;
    return true;
...
void BufferBlockStore::ensure_geometry() {
    if (ready_ || manifest_ == nullptr) return;
    block_size_ = manifest_->block_size;
    block_count_ = manifest_->block_count;
...
bool BufferBlockStore::put_block(uint32_t index, const uint8_t* data, uint32_t len) {
    if (manifest_ == nullptr) return false;
    ensure_geometry();
    if (index >= block_count_) return false;
    if (have_[index]) return true; // idempotent

    const uint32_t off = index * block_size_;
    if (off > buf_.size() || len > buf_.size() - off) return false;
...
OtaSeeder::OtaSeeder(const uint8_t* package, size_t package_len, IFrameOut& out, uint8_t session,
                     size_t fragment_size)
    : out_(out), session_(session), frag_(fragment_size ? fragment_size : 1) {
    if (frag_ > kMaxFragment) frag_ = kMaxFragment;

    ParsedPackage pkg;
    if (package_parse(package, package_len, pkg) != PackageError::Ok) return;
    ...
    for (uint32_t off = 0; off < payload_len_; off += block_size_) {
        uint32_t len = (off + block_size_ <= payload_len_) ? block_size_ : payload_len_ - off;
        leaves_.push_back(hash_leaf(payload_ + off, len));
    }
    valid_ = true;
}

void OtaSeeder::emit(const OtaFrameHeader& h, const uint8_t* payload, size_t n) {
    uint8_t frame[kMaxFramePayload];
    frame_header_pack(h, frame);
    if (n) std::memcpy(frame + kFrameHeaderLen, payload, n);
    out_.send_frame(frame, kFrameHeaderLen + n);
}

void OtaSeeder::begin() {
    if (!valid_) return;
    OtaStartInfo s{block_size_, block_count_, payload_len_, static_cast<uint16_t>(sizeof(Manifest)),
...
void OtaSeeder::send_manifest() {
    ...
    for (size_t off = 0; off < total; off += frag_) {
        ...
        OtaFrameHeader h{OtaFrameType::Manifest, session_, kIndexManifest, static_cast<uint16_t>(off),
...
void OtaSeeder::send_block(uint16_t index) {
    ...
    uint32_t len = (base + block_size_ <= payload_len_) ? block_size_ : payload_len_ - base;
    for (uint32_t off = 0; off < len; off += frag_) {
        uint32_t n = (off + frag_ <= len) ? static_cast<uint32_t>(frag_) : len - off;
        OtaFrameHeader h{OtaFrameType::Block, session_, index, static_cast<uint16_t>(off),
...
void OtaSeeder::send_proof(uint16_t index) {
    ...
    for (size_t off = 0; off < total; off += frag_) {
        size_t n = (off + frag_ <= total) ? frag_ : total - off;
        OtaFrameHeader h{OtaFrameType::Proof, session_, index, static_cast<uint16_t>(off),
                         static_cast<uint16_t>(total)};
