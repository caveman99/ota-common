...
bool BufferImageReader::read(size_t offset, uint8_t* dst, size_t n) const {
    if (offset > len_ || n > len_ - offset) return false;
...
bool esp_image_length(const IImageReader& reader, uint32_t& out_len) {
    ...
    if (!reader.read(0, header, kHeaderLen)) return false;
    if (header[0] != kEspImageMagic) return false;
    ...
    for (uint8_t i = 0; i < segment_count; ++i) {
        ...
        if (!reader.read(len, seg, kSegHeaderLen)) return false;
