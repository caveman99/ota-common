/*
 * Unity translation unit: vendored public-domain TweetNaCl plus an XEdDSA
 * verify built from its primitives. See third_party/tweetnacl/PROVENANCE.txt.
 *
 * ota_xeddsa_verify_raw converts a Curve25519 public key to Ed25519 via the RFC
 * 7748 birational map (matching CryptoEngine::curve_to_ed_pub) and then
 * verifies a standard Ed25519 signature (crypto_sign_open). This lets the
 * bare-IDF standalone loader and the host tests verify admin-key signatures
 * without the firmware's CryptoEngine.
 */
#include <string.h>
...
int ota_xeddsa_verify_raw(const unsigned char *curve_pub,
  ...
  if (crypto_sign_open(recovered, &recovered_len, sm,
                       (unsigned long long)(64 + msg_len), ed_pub) != 0) {
    return 0;
  }
