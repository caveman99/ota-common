...
static std::vector<Hash256> reduce_level(const std::vector<Hash256>& level) {
    ...
    if (i < level.size()) next.push_back(level[i]); // promote odd tail
    return next;
}
...
Hash256 merkle_root(const std::vector<Hash256>& leaves) {
    ...
    while (level.size() > 1) level = reduce_level(level);
    return level[0];
}
...
std::vector<Hash256> merkle_proof(const std::vector<Hash256>& leaves, size_t index) {
    ...
    size_t idx = index;
    while (level.size() > 1) {
        const bool idx_is_last_odd = (idx == level.size() - 1) && (level.size() % 2 == 1);
        if (!idx_is_last_odd) {
...
bool merkle_verify_block(const uint8_t* block, size_t block_len, size_t index,
                         ...
                         const Hash256& root) {
    return merkle_verify(hash_leaf(block, block_len), index, leaf_count, proof, root);
}
