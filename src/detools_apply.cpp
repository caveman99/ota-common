...
int cb_from_read(void* arg, uint8_t* buf, size_t size) {
    ...
    if (!c->base->read(c->base_pos, buf, size)) return -DETOOLS_IO_FAILED;
    c->base_pos += size;
    return 0;
}
...
int cb_from_seek(void* arg, int offset) {
    ...
    if (p < 0) return -DETOOLS_IO_FAILED;
...
int cb_patch_read(void* arg, uint8_t* buf, size_t size) {
    auto* c = static_cast<ApplyCtx*>(arg);
    if (size > c->patch_len - c->patch_pos) return -DETOOLS_IO_FAILED;
...
int detools_apply(IImageReader& base, const uint8_t* patch, size_t patch_len, IByteSink& sink) {
    ...
    return detools_apply_patch_callbacks(cb_from_read, cb_from_seek, cb_patch_read, patch_len,
                                         cb_to_write, &ctx);
}
...
int detools_apply_in_place(IFlashMem& mem, IStepJournal& journal, const uint8_t* patch,
    ...
    return detools_apply_patch_in_place_callbacks(cb_mem_read, cb_mem_write, cb_mem_erase,
                                                  cb_step_set, cb_step_get, cb_inplace_patch_read,
                                                  patch_len, &ctx);
}
