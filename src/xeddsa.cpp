...
bool xeddsa_verify(const uint8_t curve_pub[32], uint32_t from, uint32_t id, uint32_t portnum,
    ...
    if (payload == nullptr || payload_len > kXeddsaMaxPayload) return false;
    ...
    return ota_xeddsa_verify_raw(curve_pub, buf, static_cast<unsigned long>(12 + payload_len), sig) == 1;
}
